...
public class JolieToOpenApi {
    ...
    public static void main( String[] args ) {
        try {
            JolieOpenApiCommandLineParser cmdParser = JolieOpenApiCommandLineParser.create( args, JolieToOpenApi.class.getClassLoader() );
            ...
            if ("swagger".equalsIgnoreCase(format)){
                ...
                OpenApiDocumentCreator openApiDocumentCreator = new OpenApiDocumentCreator(inspector,inputPort,outputDirectory,routerHost);
                try {
                    openApiDocumentCreator.ConvertDocument();
                } catch (FaultException e) {
                    if ("RestTemplateError".equals(e.faultName())){
                        System.err.println("The file rest_template.json presents some errors please see the error.json for a detailed report");
                        ...
                        System.err.println("The mapping between rest descriptors  amd the jolie port presents some errors please see the error.json for a detailed report");
                        StringBuilder json = new StringBuilder();
                        JsUtils.valueToJsonString( e.value(), true, Type.UNDEFINED, json );
                        try( OutputStream fos = new FileOutputStream( "error.json", false ) ) {
                            OutputStreamWriter writer = new OutputStreamWriter( fos,  "UTF-8" );
